...
static inline int byte_buffer_init(ByteBuffer **buffer, FILE *fp, size_t size,
								size_t read_size) {
...
static inline int32_t byte_buffer_read_int32(ByteBuffer *buffer, enum EndianType type) {
	...
	if(type == BE) {
		ret =	buffer->m_buffer[buffer->m_offset + 0] << 24 |
				buffer->m_buffer[buffer->m_offset + 1] << 16 |
				buffer->m_buffer[buffer->m_offset + 2] <<  8 |
				buffer->m_buffer[buffer->m_offset + 3];
	} else {
		ret =	buffer->m_buffer[buffer->m_offset + 3] << 24 |
				buffer->m_buffer[buffer->m_offset + 2] << 16 |
				buffer->m_buffer[buffer->m_offset + 1] <<  8 |
				buffer->m_buffer[buffer->m_offset + 0];
	}
